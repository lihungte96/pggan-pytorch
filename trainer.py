    ...
    def train(self):
        ...
        for step in range(2, self.max_resl + 1 + 5):
            for iter in tqdm(range(0, (self.trns_tick * 2 + self.stab_tick * 2) * self.TICK, self.loader.batchsize)):
                ...
                loss_d = self.mse(self.fx[:, 0], self.real_label)
                loss_d += self.mse(self.fx_tilde[:, 0], self.fake_label)
                ...
                loss_g = self.mse(fx_tilde[:, 0], self.real_label.detach())
                loss_g.backward()
                self.opt_g.step()
